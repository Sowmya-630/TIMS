...
export default function AuthPage() {
  ...
  const handleSubmit = (e) => {
    ...
    setTimeout(() => {
      // For login, use sampleUser. For signup, use formData
      const userPayload = isLogin ? sampleUser : formData;

      dispatch({ type: 'SET_USER', payload: userPayload });
      dispatch({ type: 'SET_AUTHENTICATED', payload: true });
      dispatch({ type: 'SET_PAGE', payload: 'dashboard' });
      dispatch({ type: 'ADD_ALERT', payload: { type: 'success', message: isLogin ? 'Welcome back!' : 'Account created successfully!' }});
      setIsLoading(false);
    }, 1500);
  };
